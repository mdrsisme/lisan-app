    ...
    const fetchDetail = async () => {
      try {
        ...
        if ((result.status || result.success) && result.data) {
          ...
          setNotif({ type: "error", message: "FAQ tidak ditemukan" });
          setTimeout(() => router.push("/admin/faq/list"), 2000);
        }
      } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.status || res.success) {
        ...
        throw new Error(res.message || "Gagal memperbarui FAQ");
      }
    ...
    <AdminLayout>
      <div className="w-full max-w-7xl mx-auto space-y-8 animate-fade-in-up pb-10 px-6">
        ...
        <PageHeader
          ...
          title="Edit FAQ"
          highlight={question}
          description="Perbarui konten pertanyaan dan jawaban."
          breadcrumbs={[
        ...
        <form onSubmit={handleSubmit} className="max-w-4xl mx-auto">
          <div className="bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden">
            ...
            <div className="p-6 md:p-8 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
              <h3 className="text-lg font-bold text-slate-800">Edit Pertanyaan</h3>
