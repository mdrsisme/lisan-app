...
import { 
  ...
  AlertTriangle, X, Plus, PlayCircle
} from "lucide-react";
...
// Pastikan path ini benar atau hapus jika sudah dihandle parent layout
...
export default function AnnouncementList() {
  ...
  // Modal Video Preview
  const [videoModal, setVideoModal] = useState<{ isOpen: boolean; url: string | null }>({ isOpen: false, url: null });

  const fetchData = async () => {
    setLoading(true);
    try {
      const query = `?limit=100&search=${search}`; // Limit dibesarkan atau disesuaikan pagination
      const result = await api.get(`/announcements${query}`);
      
      // Sesuai dengan JSON Anda: result.data.data
  ...
  return (
    ...
    <div className="w-full max-w-6xl mx-auto space-y-6 animate-fade-in-up pb-20 px-6">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 p-8 text-center relative border border-slate-100">
            ...
            <p className="text-slate-500 text-sm leading-relaxed mb-8 px-4">
              Tindakan ini tidak dapat dibatalkan. Data akan hilang permanen.
            </p>
      ...
      {/* --- MODAL VIDEO PREVIEW --- */}
      {videoModal.isOpen && videoModal.url && (
              ...
              {!loading && data?.length > 0 ? (
                data.map((item) => (
                  <tr key={item.id} className="group hover:bg-[#f8fafc] transition-all duration-200">
                    ...
                    {/* Media dengan Fitur Preview */}
                    <td className="p-6 align-top">
                      <div className="flex gap-2 mt-1">
                        {item.image_url && (
                          <div className="relative group/media cursor-pointer">
                             <a href={item.image_url} target="_blank" rel="noreferrer" className="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-500 flex items-center justify-center border border-indigo-100 shadow-sm hover:bg-indigo-100 transition-colors">
                               <FileImage size={14} />
                             </a>
                          </div>
                        )}
                        ...
                        {item.video_url && (
                          <button 
                            onClick={() => setVideoModal({ isOpen: true, url: item.video_url })}
                            className="w-8 h-8 rounded-lg bg-pink-50 text-pink-500 flex items-center justify-center border border-pink-100 shadow-sm hover:bg-pink-100 transition-colors hover:scale-110 active:scale-95 group/video"
                            title="Play Video"
                          >
                            <PlayCircle size={16} className="group-hover/video:fill-pink-500/20" />
                          </button>
                        )}
