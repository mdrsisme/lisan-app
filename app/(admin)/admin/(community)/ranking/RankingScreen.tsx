...
export default function RankingScreen() {
  ...
  const fetchRankings = async () => {
    ...
    try {
      const res = await api.get(`/rankings?period=all_time&limit=50`);
      if (res.success) {
  ...
  const RankCard = ({ user, rank }: { user: any, rank: number }) => {
    ...
    return (
        <div className={`relative flex flex-col items-center p-6 rounded-[2rem] border-2 ${bgStyle} ${orderClass} ${scaleClass} transition-transform duration-300`}>
            ...
            <h3 className={`${isFirst ? 'text-lg' : 'text-base'} font-black text-center leading-tight mb-0.5 truncate w-full`}>{user.full_name}</h3>
            <p className="text-[10px] font-bold opacity-60 mb-3">@{user.username}</p>
    ...
    <AdminLayout>
      ...
      <div className="w-full max-w-6xl mx-auto space-y-6 animate-fade-in-up pb-20 px-4">
        ...
        <PageHeader
          ...
          highlight="Top 50"
          description="Peringkat pengguna terbaik sepanjang masa."
          breadcrumbs={[
        ...
        {!loading && (
            <>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 items-end pt-6 px-2 md:px-12">
                    ...
                    {rankings[1] ? (
                        <RankCard user={rankings[1]} rank={2} />
                    ) : (
                        <EmptyPodium rank={2} heightClass="h-48" />
                    )}
                    {rankings[0] ? (
                        <RankCard user={rankings[0]} rank={1} />
                    ) : (
                        <EmptyPodium rank={1} heightClass="h-56" />
                    )}

                    {rankings[2] ? (
                        <RankCard user={rankings[2]} rank={3} />
                    ) : (
                ...
                <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
                                ...
                                {rankings.slice(3).map((user) => (
                                    <tr key={user.username} className="hover:bg-slate-50 transition-colors">
                                        <td className="px-6 py-3 text-center">
                                            <span className="text-sm font-black text-slate-300">#{user.rank}</span>
                                        </td>
                    ...
                    {rankings.length === 0 && (
                        <div className="p-12 text-center">
                            ...
                            <p className="text-slate-400 text-sm font-medium">Belum ada data ranking tersedia.</p>
                        </div>
                    )}
                </div>
            </>
