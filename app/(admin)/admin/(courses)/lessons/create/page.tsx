...
export default function CreateLessonPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!moduleId) {
        setNotif({ type: "error", message: "Pilih modul terlebih dahulu" });
        setLoading(false);
        return;
    }
    ...
    try {
      ...
      if (res.success) {
        setNotif({ type: "success", message: "Pelajaran berhasil dibuat!" });
        setTimeout(() => router.push("/admin/lessons/list"), 1500);
      } else {
        throw new Error(res.message || "Gagal membuat pelajaran");
      }
